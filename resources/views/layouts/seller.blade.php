        ...
        <div class="navbar-header">
            <div class="d-flex">
                ...
                <div class="navbar-brand-box">
                    <a href="{{route('seller.home',['type_users'=>$type_users])}}" class="logo logo-light">
                        <span class="logo-sm">
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#del_item').on('click', function () {
            if (confirm("Are you sure you want to delete this item and all its related items?!")) {
                //txt = "Are you sure you want to delete this item and all its related items?!";
                var id = $('#product_id').val();
                //alert(id);
                $.ajax({
                    url: '{{route('seller.products.destroy.redeny.ajax')}}',
                    method: "get",
                    data: {id: id},
                    dataType: "json",
                    headers: {
         ...
         <?php
         $recordsOrders = \App\Models\Order::get();
         $recordProducts  = \App\Models\ProductSeller::get();
         ?>
        @foreach($recordsOrders as $record)
        $('#del_order{{$record->id}}').on('click', function () {
            if (confirm("Are you sure you want to delete this item and all its related items?! {{$record->id}}")) {
                //txt = "Are you sure you want to delete this item and all its related items?!";
                var id = $('#product_id').val();
                var state = 1;
                //alert(id);
                $.ajax({
                    url: '{{route('seller.hide.order',['id'=>$record->id,'state'=>1])}}',
                    method: "get",
        ...
        @foreach($recordProducts as $record)
        $('#del_product{{$record->id}}').on('click', function () {
            if (confirm("Are you sure you want to delete this item and all its related items?! {{$record->id}}")) {
                //txt = "Are you sure you want to delete this item and all its related items?!";
                var id = $('#del_product').val();
                var state = 1;
                //alert(id);
                $.ajax({
                    url: '{{route('seller.hide.product',['id'=>$record->id,'state'=>1])}}',
                    method: "get",
                    data: {id: id,state :state },
                    dataType: "json",
                    headers: {

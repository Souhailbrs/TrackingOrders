...
@section('content')
    <div class="row justify-content-center">
        <div class="col">
            <form action="{{ route('admin.search.requests', 'available') }}" method="get">
                <input value="" name="search" type="text" class="col-sm-8 btn-outline-primary"
                    placeholder="Search with id or name or email">
            </form>
        </div>
    </div>
                                                ...
                                                <div class="text-center">
                                                    @if ($record->state == 0)
                                                        <td><span class="badge bg-warning p-2"
                                                                style="width:100%">Pending</span></td>
                                                    @elseif($record->state == 1)
                                                        <td><span class="btn btn-success p-2"
                                                                style="width:100%">Accepted</span></td>
                                                    @elseif($record->state == 2)
                                                        <td><span class="badge  bg-primary p-2" style="width:100%;">Call
                                                                Again</span></td>
                                                    @elseif($record->state == 3)
                                                        <td><span class="badge bg-danger p-2"
                                                                style="width:100%">Ignore</span></td>
                                                    @endif
                                                        ...
                                                        <div class="btn-group" role="group">
                                                            ...
                                                            <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                                                <a class="dropdown-item col-sm-12 d-block"
                                                                    href="{{ route('change.requests.state', ['requestId' => $record->id, 'state' => 2]) }}">Call
                                                                    Again</a>
                                                                <a class="dropdown-item  col-sm-12 d-block"
                                                                    href="{{ route('change.requests.state', ['requestId' => $record->id, 'state' => 3]) }}">Ignore</a>

                                                                <a class="dropdown-item col-sm-12 d-block"
                                                                    href="{{ route('change.requests.accept', ['requestId' => $record->id]) }}">Accept</a>
                                                                <a class="dropdown-item   col-sm-12 d-block"
                                                                    data-toggle="modal"
                                                                    data-target="#exampleModalCenter{{ $record->id }}">Notes</a>
    ...
    @foreach ($records as $record)
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form class="table col-sm-12 table-bordered text-center"
                            action="{{ route('change.requests.notes') }}" method="post">
                            @csrf
                            <div class="row">
                                <div class="col-sm-12">
        ...
        <div class="modal fade" id="exampleModalCentercreate{{ $record->id }}" tabindex="-1" role="dialog"
            aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        <form class="table col-sm-12 table-bordered text-center"
                            action="{{ route('change.requests.file') }}" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="row">
                                <div class="col-sm-12">
                                    <input type="hidden" name="requestId" value="{{ $record->id }}">
